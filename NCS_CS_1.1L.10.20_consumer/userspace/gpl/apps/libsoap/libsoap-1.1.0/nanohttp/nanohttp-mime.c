...
_mime_part_begin(void *data)
{
...
#ifdef AEI_CISCO_HNAP
#ifdef WIN32
    snprintf(buffer, sizeof(buffer), "%s\\mime_%p_%d.part", cbdata->root_dir,
    ...
    if (cbdata->current_fd)
        ...
        /*CID 32009: Destination buffer too small(STRING_OVERFLOW)*/
        strncpy(cbdata->current_part->filename, buffer, sizeof(cbdata->current_part->filename) - 1);
#else
...
#ifdef AEI_COVERITY_FIX
    /*CID 31983: Resource leak(RESOURCE_LEAK)*/
    free(cbdata);
#endif
    return NULL;
  }
                             ...
                             const char *boundary, const char *dest_dir)
{
  ...
  else
  {
    ...
    log_error2("MIME parser error '%s'!",
               ...
               "Incomplete message");
#ifdef AEI_COVERITY_FIX
    /*CID 31984: Resource leak(RESOURCE_LEAK)*/
    free(cbdata);
#endif
    return NULL;
  }
