...
#ifdef AEI_CISCO_HNAP
pthread_mutex_t hnap_method_mutex = PTHREAD_MUTEX_INITIALIZER;
#endif
...
#ifdef AEI_COVERITY_FIX
  /*CID 31988: Resource leak(RESOURCE_LEAK)*/
  herror_t err;
#endif

#ifdef AEI_CISCO_HNAP
  snprintf(buffer, sizeof(buffer),template1, errmsg);
#else
  sprintf(buffer, template1, errmsg);
#endif

  snprintf(buflen, sizeof(buflen), "%d", strlen(buffer));

  httpd_set_header(conn, HEADER_CONTENT_LENGTH, buflen);
#ifdef AEI_COVERITY_FIX
  /*CID 31988: Resource leak(RESOURCE_LEAK)*/
  err = httpd_send_header(conn, 500, "INTERNAL");
  herror_release(err);
#else
...
_httpd_decode_authorization(const char *value, char **user, char **pass)
{
  ...
  unsigned char *tmp=NULL, *tmp2=NULL;
  size_t len=0;
...
#ifdef AEI_COVERITY_FIX
        /*CID 31988: Resource leak(RESOURCE_LEAK)*/
        err = httpd_send_internal_error(rconn, herror_message(status));
        herror_release(err);
#else
    ...
    else
    {
      ...
      if ((service = httpd_find_service(req->path)))
      {
        ...
        if (_httpd_authenticate_request(req, service->auth))
        {
          ...
          {
#ifdef AEI_CISCO_HNAP
              pthread_mutex_lock(&hnap_method_mutex);
#endif
            service->func(rconn, req);

#ifdef AEI_CISCO_HNAP
            pthread_mutex_unlock(&hnap_method_mutex);
#endif

            if (rconn->out &&
                    (rconn->out->type == HTTP_TRANSFER_CONNECTION_CLOSE || rconn->out->type == HTTP_TRANSFER_CONTENT_LENGTH))
            {
...
#ifdef AEI_CISCO_HNAP
  if (!rconn->out || (rconn->out && rconn->out->type == HTTP_TRANSFER_CONNECTION_CLOSE) ) {
      httpd_free(rconn);
  } else {
      httpd_free(rconn);
...
httpd_run(void)
{
  ...
  while (_httpd_run)
  {
    ...
    if (!_httpd_run) {
        log_error1("_httpd_run is 0, break");
        break;
    }
    ...
    while (_httpd_run)
    {
      ...
      case 0:
        ...
        //  log_verbose1("select timeout, descriptor is not ready");
        continue;
      case -1:
        /* got a signal? */
          log_error1("select return -1, got a signal?");
        continue;
      default:
        /* no nothing */
        break;
      ...
      {
          log_verbose1("Get FD_ISSET httpd socket");
        break;
      }
    }
...
httpd_mime_send_file(httpd_conn_t * conn, const char *content_id,
                     ...
                     const char *filename)
{
  byte_t buffer[MAX_FILE_BUFFER_SIZE]={0};
  herror_t status;
  FILE *fd;
  size_t size;
