  ...
  for (;;)
  {
    ...
    if (firstline)
    {
      ...
      if (key != NULL)
      {
        ...
        /*CID 31872: Buffer not null terminated(BUFFER_SIZE_WARNING)*/
        ...
        for (;;)
        {
          ...
          if (opt_key != NULL)
          {
            if (!(tmppair = (hpair_t *) malloc(sizeof(hpair_t))))
            {
              log_error2("malloc failed (%s)", strerror(errno));
#ifdef AEI_COVERITY_FIX
              /*CID 31985: Resource leak(RESOURCE_LEAK)*/
              hrequest_free(req);
#endif
              return NULL;
            }
...
hrequest_new_from_socket(hsocket_t *sock, hrequest_t ** out)
{
  ...
  herror_t status;
  hrequest_t *req=NULL;
  char buffer[MAX_HEADER_SIZE + 1]={0};
  attachments_t *mimeMessage=NULL;
