...
_hssl_parse_arguments(int argc, char **argv)
{
  ...
  for (i = 1; i < argc; i++)
  {
    ...
    {
      certificate = argv[i];
//      log_verbose2("certificate=%s", certificate);
    }
#ifdef AEI_WECB
    else if (!strcmp(argv[i - 1], NHTTP_ARG_PRIVATE))
    {
      privatekey = argv[i];
//      log_verbose2("privatekey=%s", privatekey);
    }
#endif
    else if (!strcmp(argv[i - 1], NHTTP_ARG_CERTPASS))
    {
      certpass = argv[i];
//      log_verbose2("certpass=%s", certpass);
    }
    else if (!strcmp(argv[i - 1], NHTTP_ARG_CA))
    {
      ca_list = argv[i];
      log_verbose2("ca_list=%s", ca_list);
    }
    else if (!strcmp(argv[i - 1], NHTTP_ARG_HTTPS))
    {
      enabled = 1;
      log_verbose1("https is enable");
    }
#ifdef AEI_WECB
    else if (!strcmp(argv[i - 1], NHTTP_ARG_CLIENT))
    {
      isclient = 1;
      log_verbose1("https is enable");
    }
...
{
#ifdef AEI_WECB
//  log_verbose5("enabled=%d, certificate=%s, privatekey=%s, certpass=%s", enabled, certificate, privatekey, certpass);
#else
...
#ifdef AEI_WECB
    log_error2("openssl error reason: %s", ERR_reason_error_string(ERR_peek_last_error()) );
    log_error2("openssl error lib: %s", ERR_lib_error_string(ERR_peek_last_error()) );
    log_error2("openssl error func: %s", ERR_func_error_string(ERR_peek_last_error()) );
#endif
...
#endif
  {
    log_error2("Cannot read key file: \"%s\"", privatekey);
#ifdef AEI_WECB
    log_error2("openssl error reason: %s", ERR_reason_error_string(ERR_peek_last_error()) );
    log_error2("openssl error lib: %s", ERR_lib_error_string(ERR_peek_last_error()) );
    log_error2("openssl error func: %s", ERR_func_error_string(ERR_peek_last_error()) );
...
#ifdef AEI_WECB
static herror_t
_hssl_client_context_init(void)
{
//  log_verbose3("client https enabled=%d, certificate=%s", enabled, certificate);
...
#ifdef AEI_WECB
  if (isclient)
      return _hssl_client_context_init();
  else
