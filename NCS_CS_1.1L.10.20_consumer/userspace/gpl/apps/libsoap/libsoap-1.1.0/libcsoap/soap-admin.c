...
_soap_admin_send_title(httpd_conn_t *conn, const char *title)
{
  ...
  /*CID 31970: Resource Leak(RESOURCE_LEAK)*/
  herror_t err;
  err = httpd_send_header(conn, 200, "OK");
  herror_release(err); 

  /*CID 31969/31697: Resource Leak(RESOURCE_LEAK)*/
  ...
  herror_release(err);
  err = http_output_stream_write_string(conn->out, title);
  herror_release(err);
  err = http_output_stream_write_string(conn->out, "<hr />");
  herror_release(err); 
#else
...
_soap_admin_list_routers(httpd_conn_t *conn)
{
  ...
  char buffer[1024]={0};
#ifdef AEI_COVERITY_FIX
  /*CID 31968/31967: Resource Leak(RESOURCE_LEAK)*/
  herror_t err;
#endif

  _soap_admin_send_title(conn, "Available routers");
#ifdef AEI_COVERITY_FIX
  /*CID 31967: Resource Leak(RESOURCE_LEAK)*/
  http_output_stream_write_string(conn->out, "<ul>");
  for (node = soap_server_get_routers(); node; node = node->next)
  {
    snprintf(buffer, sizeof(buffer), "<li><a href=\"?" SOAP_ADMIN_QUERY_ROUTER "=%s\">%s</a> - <a href=\"%s\">[Service Description]</a></li>",
	    node->context, node->context, node->context);
    err = http_output_stream_write_string(conn->out, buffer);
    herror_release(err);
  }

  err = http_output_stream_write_string(conn->out, "</ul>");
  herror_release(err);
  
  err = http_output_stream_write_string(conn->out, "</body></html>");
  herror_release(err);
#else
  http_output_stream_write_string(conn->out, "<ul>");
  for (node = soap_server_get_routers(); node; node = node->next)
...
_soap_admin_list_services(httpd_conn_t *conn, const char *routername)
{
...
#ifdef AEI_COVERITY_FIX
  /*CID 31968: Resource Leak(RESOURCE_LEAK)*/
  herror_t err;
#endif

#ifdef AEI_CISCO_HNAP
  snprintf(buffer, sizeof(buffer), "Listing Services for Router <b>%s</b>", routername);
#else
  ...
  if (!router)
  {
    err = http_output_stream_write_string(conn->out, "Router not found!");
    herror_release(err);
    err = http_output_stream_write_string(conn->out, "</body></html>");
    herror_release(err);
    return;
  ...
  {
#ifdef AEI_CISCO_HNAP
    snprintf(buffer, sizeof(buffer), "<li> [%s] (%s) </li>",
	            node->service->urn,
	            node->service->method);
#else
...
_soap_admin_handle_get(httpd_conn_t * conn, hrequest_t * req)
{
  ...
  {
#ifdef AEI_COVERITY_FIX
    /*CID 31966: Resource Leak(RESOURCE_LEAK)*/
    herror_t err;
...
_soap_admin_entry(httpd_conn_t * conn, hrequest_t * req)
{
  ...
  else
  {
    ...
    herror_t err;
    err = httpd_send_header(conn, 200, "OK");
    herror_release(err); /*CID 31965: Resource Leak(RESOURCE_LEAK)*/
    err = http_output_stream_write_string(conn->out,
              "<html>"
                  "<head>"
		  "</head>"
		  "<body>"
                  ...
                  "</body>"
              "</html>");
    herror_release(err); /*CID 31964: Resource Leak(RESOURCE_LEAK)*/
#else
    httpd_send_header(conn, 200, "OK");
    http_output_stream_write_string(conn->out,
              "<html>"
