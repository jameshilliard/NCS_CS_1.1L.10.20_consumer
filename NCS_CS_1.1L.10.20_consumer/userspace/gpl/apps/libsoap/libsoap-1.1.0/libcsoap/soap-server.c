...
_soap_server_send_env(http_output_stream_t * out, SoapEnv * env)
{
  ...
  /*CID 32980: Resource Leak(RESOURCE_LEAK)*/
  herror_t err;
#endif
...
_soap_server_send_ctx(httpd_conn_t * conn, SoapCtx * ctx)
{
  ...
  xmlBufferPtr buffer;
#ifdef AEI_CISCO_HNAP_INDENT
  xmlBufferPtr IndentBuffer=NULL;
#endif
  char strbuffer[32]={0};
  part_t *part;

#ifdef AEI_COVERITY_FIX
  /*CID 31977: Resource Leak(RESOURCE_LEAK)*/
    herror_t err;
#endif
  ...
  if ( is_peer_cisco() ) {
      buffer = xmlBufferCreate();
      xmlNodeDump(buffer, ctx->env->root->doc, ctx->env->root, 1, 0);
  } else {
...
/*  xmlKeepBlanksDefault(0);*/
  ...
  {
#ifdef AEI_CISCO_HNAP
    snprintf(strbuffer, sizeof(strbuffer), "000128590350940924234%d", counter++);
#else
    sprintf(strbuffer, "000128590350940924234%d", counter++);
#endif
#ifdef AEI_COVERITY_FIX
    /*CID 31974: Resource Leak(RESOURCE_LEAK)*/
    err = httpd_mime_send_header(conn, strbuffer, "", "text/xml", 200, "OK");
    herror_release(err);
    /*CID 31973: Resource Leak(RESOURCE_LEAK)*/
    err = httpd_mime_next(conn, strbuffer, "text/xml", "binary");
    herror_release(err);
    /*CID 31972: Resource Leak(RESOURCE_LEAK)*/
    err = http_output_stream_write_string(conn->out,
                                    (const char *) xmlBufferContent(buffer));
    herror_release(err);
#else
    ...
    {
#ifdef AEI_COVERITY_FIX
      /*CID 31975: Resource Leak(RESOURCE_LEAK)*/
      err = 
#endif
  ...
  else
  {
    ...
    xpathObj = xmlXPathEvalExpression("//Fault", xpathCtx);
        ...
        doc = xmlParseDoc((xmlChar *) xmlBufferContent(buffer));
        root_node = xmlDocGetRootElement(doc);
        IndentBuffer = xmlBufferCreate();
        xmlNodeDump(IndentBuffer, doc, root_node, 1, 1);
        snprintf(buflen, sizeof(buflen), "%d", xmlBufferLength(IndentBuffer)+strlen(_SOAP_XML_VERSION_HEADER));

    #else
        snprintf(buflen, sizeof(buflen), "%d", xmlBufferLength(buffer)+strlen(_SOAP_XML_VERSION_HEADER));
    #endif

#else
    snprintf(buflen, sizeof(buflen), "%d", xmlBufferLength(buffer));
#endif
    ...
    {
#ifdef AEI_COVERITY_FIX
      /*CID 31976: Resource Leak(RESOURCE_LEAK)*/
      err = 
#endif
    ...
    xmlerr = http_output_stream_write_string(conn->out, _SOAP_XML_VERSION_HEADER);
    herror_release(xmlerr);
    log_verbose2("%s", _SOAP_XML_VERSION_HEADER);

    #ifdef AEI_CISCO_HNAP_INDENT
        log_info2("\n%s",  (char *) xmlBufferContent(IndentBuffer));

        xmlerr = http_output_stream_write_string(conn->out, (const char *) xmlBufferContent(IndentBuffer));
        herror_release(xmlerr);
    #else

        log_info2("\n%s",  (char *) xmlBufferContent(buffer));

        xmlerr = http_output_stream_write_string(conn->out, (const char *) xmlBufferContent(buffer));
        herror_release(xmlerr);
    #endif
...
_soap_server_send_description(httpd_conn_t *conn, xmlDocPtr wsdl)
{
  ...
  xmlBufferPtr buf;
#ifdef AEI_COVERITY_FIX
  /*CID 31979: Resource Leak(RESOURCE_LEAK)*/
  herror_t err;
#endif
  buf = xmlBufferCreate();
...
#ifdef AEI_COVERITY_FIX
  /*CID 31979: Resource Leak(RESOURCE_LEAK)*/
  err = httpd_send_header(conn, 200, "OK");
  herror_release(err);
  /*CID 31978: Resource Leak(RESOURCE_LEAK)*/
  err = http_output_stream_write_string(conn->out, xmlBufferContent(buf));
  herror_release(err);
#else
  httpd_send_header(conn, 200, "OK");
...
soap_server_entry(httpd_conn_t * conn, hrequest_t * req)
{
  ...
  {
#ifdef AEI_COVERITY_FIX
    /*CID 31982: Resource Leak(RESOURCE_LEAK)*/
    herror_t err;
    err = httpd_send_header(conn, 200, "OK");
    herror_release(err);
    /*CID 31981: Resource Leak(RESOURCE_LEAK)*/
    err = http_output_stream_write_string(conn->out,
              "<html>"
                  "<head>"
		  "</head>"
  ...
  else
  {
    ...
    else
    {
      ...
      {
#ifdef AEI_CISCO_HNAP
        snprintf(buffer, sizeof(buffer),"URN '%s' not found", urn);
#else
      ...
      else
      {
        ...
        {
#ifdef AEI_CISCO_HNAP
          snprintf(buffer,sizeof(buffer),"Service returned following error message: '%s'",
                  herror_message(err));
#else
...
#ifdef AEI_COVERITY_FIX
          /*CID 32094: Resource Leak(RESOURCE_LEAK)*/
        ...
        {
#ifdef AEI_CISCO_HNAP
          snprintf(buffer,sizeof(buffer), "Service '%s' returned no envelope", urn);
#else
