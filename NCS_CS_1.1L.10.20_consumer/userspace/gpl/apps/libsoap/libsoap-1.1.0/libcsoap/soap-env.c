...
#define _SOAP_CISCO_MSG_TEMPLATE_ \
...
enum hnap_custom_t {
    PEER_CUSTOM_NULL=0,
    PEER_CUSTOM_CISCO,
    PEER_CUSTOM_TECHNICOLOR,
};

static int peer_custom_type = PEER_CUSTOM_NULL;
...
int is_peer_cisco()
{
    if (PEER_CUSTOM_CISCO == peer_custom_type)
        return 1;
    else
        return 0;
}

int check_xml_type(const char *pstr, int readed)
{
    char tmpbuf[4096]={0};

    if (peer_custom_type)
        return peer_custom_type;

    if (pstr && readed >1) {
        snprintf(tmpbuf, sizeof(tmpbuf)>readed? sizeof(tmpbuf)-1:readed, "%s", pstr);
      //  log_info2("check xml: %s", tmpbuf);
#if 0
        if (strstr(tmpbuf,"soap:encodingStyle")) {
            log_verbose1("has soap:encodingStyle, Is Cisco Peer HNAP");
            peer_custom_type = PEER_CUSTOM_CISCO;
        }
#endif
        if (strchr(tmpbuf,'\n')) {
            log_info1("Is Cisco Peer HNAP");
            peer_custom_type = PEER_CUSTOM_CISCO;
        }   else {
            log_info1("Is Technicolor Peer HNAP");
            peer_custom_type = PEER_CUSTOM_TECHNICOLOR;
        }
    }
...
#else
    xmlStrPrintf(buffer, sizeof(buffer), BAD_CAST _SOAP_CISCO_MSG_TEMPLATE_,
            soap_env_ns,
            BAD_CAST method,
            BAD_CAST urn,
            BAD_CAST method);
...
soap_env_new_from_stream(http_input_stream_t * in, SoapEnv ** out)
{
  ...
  if ( is_peer_cisco() ) {
      ...
      if (! root_node) {
          log_error1("xmlDocGetRootElement failed");
          return soap_env_new_from_doc(doc, out);
      }

      buffer = xmlBufferCreate();
      if (! buffer) {
          log_error1("xmlBufferCreate failed");
          return soap_env_new_from_doc(doc, out);
      }

      if (xmlNodeDump(buffer, doc, root_node, 1, 0)==-1) {
          log_error1("xmlNodeDump failed");
          xmlBufferFree(buffer);
          return soap_env_new_from_doc(doc, out);
      }

      log_info2("Get HNAP buffer:\n%s", (char *) xmlBufferContent(buffer));

      format_doc = xmlParseDoc((xmlChar *) xmlBufferContent(buffer));

      if (! format_doc) {
          log_error1("xmlParseDoc failed");
          xmlBufferFree(buffer);
          return soap_env_new_from_doc(doc, out);
      }
      xmlBufferFree(buffer);
      ...
      return soap_env_new_from_doc(format_doc, out);
  } else {
      root_node = xmlDocGetRootElement(doc);
      if (! root_node) {
          log_error1("xmlDocGetRootElement failed");
