                   ...
                   const char *soap_action)
{
  ...
  if (!call->attachments)
  {
    ...
    sprintf(tmp, "%d", (int) strlen(content)+ strlen(_SOAP_XML_VERSION_HEADER));
#else
...
#ifdef AEI_CISCO_HNAP
    if ((status = http_output_stream_write_string(conn->out, _SOAP_XML_VERSION_HEADER	)) != H_OK)
    {
      httpc_close_free(conn);
      xmlBufferFree(buffer);
      return status;
