...
static len_and_sockaddr* str2sockaddr_ext(
		const char *host, int port,
IF_FEATURE_IPV6(sa_family_t af,)
		int ai_flags, int exit_or_not,int *error_type)
{
IF_NOT_FEATURE_IPV6(sa_family_t af = AF_INET;)
	...
	if (ENABLE_FEATURE_IPV6 && host[0] == '[') {
		...
		if (!cp || (cp[1] != ':' && cp[1] != '\0')) {
			...
			if ((ai_flags & DIE_ON_ERROR)&&(exit_or_not))
				xfunc_die();
                     *error_type=PING_HOST_ERROR;
			return NULL;
		}
	} else {
		cp = strrchr(host, ':');
...
{
    int error_type=-1;
    return str2sockaddr_ext(host, port, af, ai_flags, 1, &error_type);
}
...
len_and_sockaddr* FAST_FUNC xhost_and_af2sockaddr_ext(const char *host, int port, sa_family_t af,int *error_type)
{
    return str2sockaddr_ext(host, port, af, DIE_ON_ERROR,0,error_type);
}
#endif
