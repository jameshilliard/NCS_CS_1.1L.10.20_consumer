 ...
 */
#define CONFIG_AEI_GETTY_LOGIN_ARGS
...
struct options {
    ...
    const char *login_args;         /* Extra args pass to login program*/
#endif
};
...
#if defined (CONFIG_AEI_GETTY_LOGIN_ARGS)
static const char opt_string[] ALIGN1 = "I:LH:f:hil:mt:wna:";
#else
...
#if defined (CONFIG_AEI_GETTY_LOGIN_ARGS)
#define F_LOGINARGS     (1 << 11)       /* -a login args */
#endif
...
#if defined (CONFIG_AEI_GETTY_LOGIN_ARGS)
    if (options.flags & F_LOGINARGS) {
	    BB_EXECLP(options.login, options.login, logname, options.login_args, NULL);
    } else {
