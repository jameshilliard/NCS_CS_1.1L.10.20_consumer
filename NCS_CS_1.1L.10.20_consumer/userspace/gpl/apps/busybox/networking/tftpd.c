...
#define FILE_TFTPD_LOG  "/var/run/tftpd.log"
#endif
...
#else
#if defined (AEI_WECB)
#define FORK() fork() /*See issue 203607 for reason */
#else
#define FORK() vfork()
#endif
#endif
...
tftpd_send (int peer, struct tftphdr *first_pkt, int pkt_len, int buffersize)
{
   ...
   snprintf(cmd,sizeof(cmd),"echo \"tftpd send file %s\" >> %s",list[0],FILE_TFTPD_LOG);
   system(cmd);
   file = fopen (path, "r");
#else
   file = fopen (list[0], "r");
...
#if defined (AEI_WECB)
   snprintf(cmd,sizeof(cmd),"echo \"tftpd send file %s start\" >> %s",list[0],FILE_TFTPD_LOG);
   system(cmd);
#endif
   /* Send the file! */
   ...
   fclose(file);
#if defined (AEI_WECB)
   snprintf(cmd,sizeof(cmd),"echo \"tftpd send file %s end\" >> %s",list[0],FILE_TFTPD_LOG);
   system(cmd);
#endif
   common_exit(0);
}
...
// brcm   FILE *file=NULL;
   ...
   int listcount;
    ...
    fprintf(stderr,"tftpd_receive in\n");
...
// brcm   fclose(file);
   ...
   printf("Total upload size: %d bytes\n", uploadSize);
    ...
    fprintf(stderr,"tftpd_receive out\n");
              ...
              int port UNUSED_PARAM)
{
   ...
   fprintf(stderr,"tftpd_daemon in\n");
