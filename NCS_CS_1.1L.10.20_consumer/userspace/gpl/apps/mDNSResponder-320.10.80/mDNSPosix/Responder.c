...
static void PrintUsage()
{
    fprintf(stderr, 
#ifdef AEI_WECB
            "Usage: %s [-v level ] [-r] [-n name] [-t type] [-d domain] [-p port] [-f file] [-b] [-P pidfile] [-x name=val ...] [-e interfaces]\n",
#else
    ...
    fprintf(stderr, "          -e exclude the 'interfaces', while using the function get_ifi_info to get a list of interfaces.\n");
    fprintf(stderr, "             if more than one interface list, MUST be separated with ',' each other.\n");
#else
	fprintf(stderr, "             all subsequent arguments after -x are treated as name=val pairs.\n");
#endif
}
...
#ifdef AEI_WECB
extern char gInterface[IFI_NUM][IFI_NAME_SIZE+1];
#endif
    ...
    // listed above.
{
    ...
    do {
#ifdef AEI_WECB
        ch = getopt(argc, argv, "v:rn:t:d:p:e:f:dP:bx");
#else
        ...
        if (ch != -1) {
            switch (ch) {
                ...
                case 'e':
                    {
                        int i = 0, size;
                        char *interfaces = optarg;
                        char *separate = NULL;
                        while((i < IFI_NUM) && (separate = strchr(interfaces, ',')) != NULL)
                        {
                            size = (int)(separate-interfaces);
                            size = (size < IFI_NAME_SIZE) ? size : IFI_NAME_SIZE;
                            strncpy(gInterface[i], interfaces, size);
                            separate++;
                            interfaces = separate;
                            gInterface[i][size] = '\0';
                            i++;
