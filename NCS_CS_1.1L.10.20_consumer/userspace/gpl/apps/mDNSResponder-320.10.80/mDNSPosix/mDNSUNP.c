...
#include <stdio.h>
#ifdef AEI_WECB
const char gInterface[IFI_NUM][IFI_NAME_SIZE+1];
#endif
...
#ifdef AEI_WECB
        need_exclude = count = 0;
        while((count < IFI_NUM) && strcmp(gInterface[count], ""))
        {
            if (!strcmp(ifr->ifr_name, gInterface[count]))
            {
                need_exclude = 1;
                break;
            }
