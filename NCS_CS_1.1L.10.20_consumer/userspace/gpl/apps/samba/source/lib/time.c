...
#ifdef AEI_WECB
void AEI_GetTimeOfDay(struct timeval *tval)
{
	struct timespec tmp_tval;
#if 0 // for debug
	struct timeval debug_tval;
#endif

	clock_gettime(CLOCK_MONOTONIC, &tmp_tval);

	tval->tv_sec = tmp_tval.tv_sec;
	tval->tv_usec = tmp_tval.tv_nsec/1000;

#if 0 // for deug
#ifdef HAVE_GETTIMEOFDAY_TZ
	gettimeofday(&debug_tval,NULL);
#else
	gettimeofday(&debug_tval);
#endif

	fprintf(stderr, "******************* GetTimeOfDay: %ds, %dms\n", debug_tval.tv_sec, debug_tval.tv_usec/1000);
	fprintf(stderr, "******************* %p, AEI_GetTimeOfDay: %ds, %dms\n", tval, tval->tv_sec, tval->tv_usec/1000);
#endif
}
#endif
