...
#ifdef AEI_BRCM_CORE
	/* cli depend on libcms_core.so */
	envp[j++] = find_env("LD_LIBRARY_PATH", "LD_LIBRARY_PATH=/lib/public:/lib/private:/lib/gpl:/lib");
#endif
...
void run_script(struct dhcpMessage *packet, const char *name)
{
...
#ifdef AEI_WECB
    if (client_config.script == NULL || name == NULL)
#else
    ...
    if (strlen(local_ip) > 0 && strcmp(name, "deconfig")) {
        char cmd[256]="";
