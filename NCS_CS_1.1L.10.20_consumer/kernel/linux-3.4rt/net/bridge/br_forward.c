...
#endif
{
	...
	if ( pigmp != NULL )
	{
		...
		else
		{
			...
			{
				struct net_bridge_port *src_port = NULL;
				static int wan_port_no = -1;
				int src_port_no = -1;

				src_port = br_port_get_rcu(skb->dev);
				if(src_port != NULL)
				{
					src_port_no = src_port->port_no;
					if(pigmp->type == IGMP_HOST_MEMBERSHIP_QUERY)
					{
						if(wan_port_no != src_port_no)
						{
							wan_port_no = src_port_no;
							printk("%s() LINE %d change WAN port to 0x%x\n", __func__, __LINE__, wan_port_no);
						}
					}
					else
					{
						struct net_bridge_port *dst_port = NULL;
						if(wan_port_no == src_port_no)
						{
							// printk("%s() LINE %d droped package pigmp->type 0x%x from port 0x%x\n", __func__, __LINE__, pigmp->type, src_port_no);
							// should not forwarding JOIN/REPORT/LEAVE package from WAN port to LAN port
							return 0;
						}

						dst_port = br_port_get_rcu(p->dev);
						if(dst_port != NULL)
						{
							if(wan_port_no != -1 && dst_port->port_no != wan_port_no)
							{
								// printk("%s() LINE %d droped package pigmp->type 0x%x from port 0x%x to port 0x%x\n", __func__, __LINE__, pigmp->type, src_port_no, dst_port->port_no);
								// should not forwarding JOIN/REPORT/LEAVE package from LAN port to LAN port
								return 0;
							}
						}

						// should forwarding JOIN/REPORT/LEAVE package from LAN port to WAN port
						// printk("%s() LINE %d forwarding pigmp->type 0x%x from port 0x%x to port 0x%x\n", __func__, __LINE__, pigmp->type, src_port_no, dst_port->port_no);
					}
				}
			}
#else
