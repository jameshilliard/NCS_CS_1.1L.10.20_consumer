...
{
	bool armpll_fast = false;
	int freqMHz, ndiv, pdiv;
	uint32 policy;
	...
	if (armpll_fast == true) {
		freqMHz = 25 * ndiv / pdiv;
	} else {
		/* TODO! found out the frequency for armpll_slow, even though
		 * we are not using it. */
		return 0;
	}
	return FREQ_MHZ(freqMHz);
}
...
{
	int ndiv = 0, policy = 0, ret = 0;
	struct clk *axi_clk, *apb_clk;
	unsigned int freqMHz = freqHz / 1000000;

	if (freqMHz < 200 || freqMHz > BCM63138_MAX_CORE_FREQ || (freqMHz % 25)) {
		printk("\tInvalid cpu frequency %d MHz, the supported range is "
				"between 200MHz to %d MHz and in the "
				"multiple of 25MHz\n", freqMHz, BCM63138_MAX_CORE_FREQ);
		return -EINVAL;
	}

	/* enable write access the ARM clk manager */
	ARMCFG->proc_clk.wr_access |= (ARM_PROC_CLK_WR_ACCESS_PASSWORD << ARM_PROC_CLK_WR_ACCESS_PASSWORD_SHIFT) |
		ARM_PROC_CLK_WR_ACCESS_CLKMGR_ACC;
	ndiv = 2 * freqMHz / 25;

	if (freqMHz > 200) {
		/* set clk divider and enable pll */
		ARMCFG->proc_clk.pllarma = (ARMCFG->proc_clk.pllarma &
				~(ARM_PROC_CLK_PLLARMA_PDIV_MASK | ARM_PROC_CLK_PLLARMA_NDIV_MASK |
					ARM_PROC_CLK_PLLARMA_PWRDWN_SWOVRRIDE_MASK)) |
			(2 << ARM_PROC_CLK_PLLARMA_PDIV_SHIFT) | (ndiv << ARM_PROC_CLK_PLLARMA_NDIV_SHIFT) |
	...
	/* set the freq policy */
	policy = (freqMHz == 200) ? ARM_PROC_CLK_POLICY_FREQ_SYSCLK : ARM_PROC_CLK_POLICY_FREQ_ARMPLL_FAST;
	ARMCFG->proc_clk.policy_freq = (ARMCFG->proc_clk.policy_freq & ~ARM_PROC_CLK_POLICY_FREQ_MASK) |
		(policy << ARM_PROC_CLK_POLICY3_FREQ_SHIFT) |
		(policy << ARM_PROC_CLK_POLICY2_FREQ_SHIFT) |
		(policy << ARM_PROC_CLK_POLICY1_FREQ_SHIFT) |
		(policy << ARM_PROC_CLK_POLICY0_FREQ_SHIFT);

	/* enabled hardware clock gating */
	ARMCFG->proc_clk.core0_clkgate = (ARMCFG->proc_clk.core0_clkgate &
			~ARM_PROC_CLK_CORE0_CLKGATE_GATING_SEL_MASK) |
		(ARM_PROC_CLK_CORE0_CLKGATE_GATING_SEL_HW << ARM_PROC_CLK_CORE0_CLKGATE_GATING_SEL_SHIFT);
	ARMCFG->proc_clk.core1_clkgate = (ARMCFG->proc_clk.core1_clkgate &
			~ARM_PROC_CLK_CORE1_CLKGATE_GATING_SEL_MASK) |
		(ARM_PROC_CLK_CORE1_CLKGATE_GATING_SEL_HW << ARM_PROC_CLK_CORE1_CLKGATE_GATING_SEL_SHIFT);
	ARMCFG->proc_clk.arm_switch_clkgate = (ARMCFG->proc_clk.arm_switch_clkgate &
			~ARM_PROC_CLK_SWITCH_CLKGATE_GATING_SEL_MASK) |
		(ARM_PROC_CLK_SWITCH_CLKGATE_GATING_SEL_HW << ARM_PROC_CLK_SWITCH_CLKGATE_GATING_SEL_SHIFT);
	ARMCFG->proc_clk.arm_periph_clkgate = (ARMCFG->proc_clk.arm_periph_clkgate &
			~ARM_PROC_CLK_PERIPH_CLKGATE_GATING_SEL_MASK) |
		(ARM_PROC_CLK_PERIPH_CLKGATE_GATING_SEL_HW << ARM_PROC_CLK_PERIPH_CLKGATE_GATING_SEL_SHIFT);
	ARMCFG->proc_clk.apb0_clkgate = (ARMCFG->proc_clk.apb0_clkgate &
			~ARM_PROC_CLK_APB0_CLKGATE_GATING_SEL_MASK) |
		(ARM_PROC_CLK_APB0_CLKGATE_GATING_SEL_HW << ARM_PROC_CLK_APB0_CLKGATE_GATING_SEL_SHIFT);
	...
	axi_clk->rate = FREQ_MHZ(freqMHz) >> 1;
	apb_clk->rate = FREQ_MHZ(freqMHz) >> 2;
