...
static int arm_wfi_allowed = 1; // adminstratively allowed

__attribute__ (( aligned(16),hot ))
static void bcm63xx_arm_pm_idle(void) {
	local_irq_enable();
	do {	// fit in 64-byte cache-line
		asm("nop; nop; nop; nop"); // 16-bytes
		asm("nop; nop; nop; nop"); // 16-bytes
		asm("nop; nop; nop; nop"); // 16-bytes
	} while (!need_resched());
}
...
static int core_set_freq(void *p)
{
	...
	if (shift != 0) {
		...
		// 75us spike protection only needed when going to 1500MHz?
		// enable | microseconds | 75
		PMC->ctrl.gpTmr2Ctl = (1 << 31) | (1 << 29) | 75;
		while (PMC->ctrl.gpTmr2Ctl & (1 << 31));
