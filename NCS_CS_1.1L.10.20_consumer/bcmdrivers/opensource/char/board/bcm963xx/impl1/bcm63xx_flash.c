    ...
    if( !IS_ERR_OR_NULL(mtd0) )
    {
        ...
        else
        {
...
#if defined(AEI_WECB)
            unsigned char tmp_image_version[sizeof(pnd->ImageVersion2)];
            memcpy(tmp_image_version, pnd->ImageVersion2, sizeof(pnd->ImageVersion2));
#endif
           ...
           if( (wt.wfiFlags&WFI_FLAG_HAS_PMC) == 0 )
           {
            ...
            {
            case 1:
                memcpy(pnd->ImageVersion1, tmp_image_version, sizeof(pnd->ImageVersion2));
                break;
            case 2:
                memcpy(pnd->ImageVersion2, tmp_image_version, sizeof(pnd->ImageVersion2));
                break;
            default:
                break;
            }
            printk("%s() LINE %d: partition %d, image_version %s\n", __func__, __LINE__, partition, tmp_image_version);
#endif
