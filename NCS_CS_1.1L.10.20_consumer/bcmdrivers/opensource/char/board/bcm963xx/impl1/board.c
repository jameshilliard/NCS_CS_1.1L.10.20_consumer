...
#ifdef CONFIG_ACT_NL_KERNEL
#include "nl_kernel/nl_kernel.h"
#endif
...
#ifdef CONFIG_ACT_NL_KERNEL
static nl_button_state get_button_gpio(unsigned int button_id)
{
	nl_button_state state = NL_BUTTON_KEYUP;

	switch(button_id){
		case NL_BUTTON_ID_RESET:
			break;
		case NL_BUTTON_ID_WPS:
			state = sesBtn_pressed();
			break;
		default:
			break;
	}
...
static button_op_cb g_button_op_cb = {
	.get_button_gpio = get_button_gpio,
};
extern led_op_cb g_led_op_cb;
#endif
...
#ifdef CONFIG_ACT_NL_KERNEL
	act_set_led_callback(&g_led_op_cb);
	act_set_button_callback(&g_button_op_cb);
#endif
...
int kerSysGetSequenceNumber(int imageNumber)
{
    ...
    printk("%s(): LINE %d imageNumber %d seqNumber %d\n", __func__, __LINE__, imageNumber, seqNumber);
...
#if defined(AEI_WECB)
        aei_upgrade_partition = getBootedValue(1) == BOOTED_PART1_IMAGE? 2: 1;
#endif
        ...
        else
        {
                ...
                NP_ROOTFS_1 + partition - 1] )
            {
                ...
                if( pnoReboot )
                    *pnoReboot = 0;
#if defined(AEI_WECB)
                aei_upgrade_partition = aei_upgrade_partition == 1 ? 1 : 2;
#endif
            }

#if defined(AEI_WECB)
            ret = kerSysBcmNandImageSet(rootfs_part, string, size,
                (pnoReboot) ? *pnoReboot : 0, aei_upgrade_partition);
#else
            ret = kerSysBcmNandImageSet(rootfs_part, string, size,
...
typedef enum
{
    ...
    AEI_NVPARAM_WPA_KEY,
    AEI_NVPARAM_WPS_AP_PIN,
#ifdef AEI_WECB
    AEI_NVPARAM_ADMIN_PASSWORD,
    AEI_NVPARAM_CONSOLE_ACCESS,
#endif
    ...
    AEI_NVPARAM_CFE_CONSOLE_PASSWORD,
#endif
...
static int aei_getNvramValueByParaName(AEI_NVRAM_PARAM_INDEX pIndex, char* parValue, int len)
{
    ...
    {
    case AEI_NVPARAM_SN:
        strlcpy(parValue, nvramData.BoardSN, len);
        break;
    ...
    case AEI_NVPARAM_WPA_KEY:
        ...
        break;
    case AEI_NVPARAM_WPS_AP_PIN:
        strlcpy(parValue, nvramData.WpsAPPin, len);
        break;
#ifdef AEI_WECB
    case AEI_NVPARAM_ADMIN_PASSWORD:
        strlcpy(parValue, nvramData.AdminPassword, len);
        break;
    case AEI_NVPARAM_CONSOLE_ACCESS:
        *(unsigned int *)parValue = nvramData.ulConsoleAccess;
        break;
#endif
#ifdef AEI_FACTORY_TEST
...
#endif
#ifdef AEI_CONSOLE_PASSWD_IN_NVRAM
    case AEI_NVPARAM_CFE_CONSOLE_PASSWORD:
        if(len < sizeof(nvramData.cfeConsolePassword1))
        {
            strlcpy(parValue, nvramData.cfeConsolePassword1, len);
        }
        else
        {
            strncpy(parValue, nvramData.cfeConsolePassword1, sizeof(nvramData.cfeConsolePassword1) - 1);
            strlcpy(parValue + sizeof(nvramData.cfeConsolePassword1) - 1, nvramData.cfeConsolePassword2, len - (sizeof(nvramData.cfeConsolePassword1) - 1));
        }
        break;
#endif
    default:
                       ...
                       unsigned int command, unsigned long arg )
{
            ...
            case NVRAM:
            {
                ...
                if ( !strncmp(ctrlParms.string, "WLANFEATURE", 11 ) ) { //Wlan Data data
                   ...
                   int t_strlen=ctrlParms.strLen-sizeof(NVRAM_AEI_WPS_AP_PIN_TAG);
                    if(t_strlen>sizeof(pNvramData->WpsAPPin)-1)
                        t_strlen=sizeof(pNvramData->WpsAPPin)-1;
                   memset(pNvramData->WpsAPPin,0,sizeof(pNvramData->WpsAPPin));
                   //printk("\npNvramDate-WpsAPPin=%s.t_strlen=%d ,ctlParms.string=%s\n\n",pNvramData->WpsAPPin,t_strlen,ctrlParms.string+sizeof(NVRAM_AEI_WPS_AP_PIN_TAG));
                   memcpy(pNvramData->WpsAPPin,(unsigned char *)(ctrlParms.string+sizeof(NVRAM_AEI_WPS_AP_PIN_TAG)),t_strlen);
                   writeNvramDataCrcLocked(pNvramData);
                }
                  ...
                  // printk("\npNvramDate-AdminPassword=%s.t_strlen=%d ,ctlParms.string=%s\n\n",pNvramData->AdminPassword,t_strlen,ctrlParms.string+sizeof(NVRAM_AEI_ADMIN_PASSWORD_TAG));
                   memcpy(pNvramData->AdminPassword,(unsigned char *)(ctrlParms.string+sizeof(NVRAM_AEI_ADMIN_PASSWORD_TAG)),t_strlen);
                   writeNvramDataCrcLocked(pNvramData);
                }
                else if( !strncmp(ctrlParms.string, NVRAM_AEI_CONSOLE_ACCESS_TAG, sizeof(NVRAM_AEI_CONSOLE_ACCESS_TAG) ) ){// console access flag
                   pNvramData->ulConsoleAccess=simple_strtol((unsigned char *)(ctrlParms.string+sizeof(NVRAM_AEI_CONSOLE_ACCESS_TAG)),NULL,0);
                   writeNvramDataCrcLocked(pNvramData);
                }
                   ...
                   int t_strlen=ctrlParms.strLen-sizeof(NVRAM_AEI_SN_TAG);
                    if(t_strlen>sizeof(pNvramData->BoardSN)-1)
                        t_strlen=sizeof(pNvramData->BoardSN)-1;
                   memset(pNvramData->BoardSN,0,sizeof(pNvramData->BoardSN));
                   //printk("\npNvramDate-BoardSN=%s.t_strlen=%d ,ctlParms.string=%s\n\n",pNvramData->BoardSN,t_strlen,ctrlParms.string+sizeof(NVRAM_AEI_ADMIN_PASSWORD_TAG));
                   memcpy(pNvramData->BoardSN,(unsigned char *)(ctrlParms.string+sizeof(NVRAM_AEI_SN_TAG)),t_strlen);
                   writeNvramDataCrcLocked(pNvramData);
                }
                   ...
                   int t_strlen=ctrlParms.strLen-sizeof(NVRAM_AEI_BASE_MAC_ADDRESS_TAG);
                    if(t_strlen>sizeof(pNvramData->ucaBaseMacAddr)-1)
                        t_strlen=sizeof(pNvramData->ucaBaseMacAddr)-1;
                   //printk("\npNvramDate-ucaBaseMacAddr=%s.t_strlen=%d ,ctlParms.string=%s\n\n",pNvramData->ucaBaseMacAddr,t_strlen,ctrlParms.string+sizeof(NVRAM_AEI_BASE_MAC_ADDRESS_TAG));
...
#endif        
#ifdef AEI_CONSOLE_PASSWD_IN_NVRAM
                else if( !strncmp(ctrlParms.string, NVRAM_AEI_CFE_CONSOLE_PASSWORD_TAG, sizeof(NVRAM_AEI_CFE_CONSOLE_PASSWORD_TAG) )){ // CFE Console Password
                   int t_strlen=ctrlParms.strLen-sizeof(NVRAM_AEI_CFE_CONSOLE_PASSWORD_TAG);
                    if(t_strlen>(sizeof(pNvramData->cfeConsolePassword1)-1 + sizeof(pNvramData->cfeConsolePassword2)-1))
                        t_strlen=sizeof(pNvramData->cfeConsolePassword1)-1 + sizeof(pNvramData->cfeConsolePassword2)-1;
                   //printk("\npNvramDate-cfeConsolePassword=%s%s.t_strlen=%d ,ctlParms.string=%s\n\n",pNvramData->cfeConsolePassword1,pNvramData->cfeConsolePassword2,t_strlen,ctrlParms.string+sizeof(NVRAM_AEI_CFE_CONSOLE_PASSWORD_TAG));
                   memset(pNvramData->cfeConsolePassword1,0,sizeof(pNvramData->cfeConsolePassword1));
                   memset(pNvramData->cfeConsolePassword2,0,sizeof(pNvramData->cfeConsolePassword2));
                   if(t_strlen < (sizeof(pNvramData->cfeConsolePassword1)-1))
                   {
                       memcpy(pNvramData->cfeConsolePassword1,(unsigned char *)(ctrlParms.string+sizeof(NVRAM_AEI_CFE_CONSOLE_PASSWORD_TAG)),t_strlen);
                   }
                   else
                   {
                       memcpy(pNvramData->cfeConsolePassword1,(unsigned char *)(ctrlParms.string+sizeof(NVRAM_AEI_CFE_CONSOLE_PASSWORD_TAG)), (sizeof(pNvramData->cfeConsolePassword1)-1));
                       memcpy(pNvramData->cfeConsolePassword2,(unsigned char *)(ctrlParms.string+sizeof(NVRAM_AEI_CFE_CONSOLE_PASSWORD_TAG) + (sizeof(pNvramData->cfeConsolePassword1)-1)),t_strlen - (sizeof(pNvramData->cfeConsolePassword1)-1));
                   }
                   writeNvramDataCrcLocked(pNvramData);
                }
#endif
    ...
    case BOARD_IOCTL_GET_HW_VERSION:
       if (copy_from_user((void*)&ctrlParms, (void*)arg, sizeof(ctrlParms)) == 0)
       {
           if( ctrlParms.string )
           {
               ...
               __copy_to_user(ctrlParms.string, paramValue, 9 - 1);
           }
    ...
    case BOARD_IOCTL_GET_WPAKEY:
       if (copy_from_user((void*)&ctrlParms, (void*)arg, sizeof(ctrlParms)) == 0)
       {
           if( ctrlParms.string )
           {
               char paramValue[65] = {0};

               aei_getNvramValueByParaName(AEI_NVPARAM_WPA_KEY, paramValue, sizeof(paramValue));

               __copy_to_user(ctrlParms.string, paramValue, 65 - 1);
           }
    ...
    case BOARD_IOCTL_GET_WPS_AP_PIN:
       if (copy_from_user((void*)&ctrlParms, (void*)arg, sizeof(ctrlParms)) == 0)
       {
           if( ctrlParms.string )
           {
               ...
               aei_getNvramValueByParaName(AEI_NVPARAM_WPS_AP_PIN, paramValue, sizeof(paramValue));
...
#ifdef AEI_WECB
    case BOARD_IOCTL_GET_ADMIN_PASSWORD:
       if (copy_from_user((void*)&ctrlParms, (void*)arg, sizeof(ctrlParms)) == 0)
       {
           if( ctrlParms.string )
           {
               ...
               aei_getNvramValueByParaName(AEI_NVPARAM_ADMIN_PASSWORD, paramValue, sizeof(paramValue));
       ...
       break;
    case BOARD_IOCTL_GET_CONSOLE_ACCESS:
       if (copy_from_user((void*)&ctrlParms, (void*)arg, sizeof(ctrlParms)) == 0)
       {
           if( ctrlParms.string )
           {
               ...
               aei_getNvramValueByParaName(AEI_NVPARAM_CONSOLE_ACCESS, paramValue, sizeof(paramValue));
...
#endif
#ifdef AEI_FACTORY_TEST
    case BOARD_IOCTL_GET_BURN_IN_TEST:
       if (copy_from_user((void*)&ctrlParms, (void*)arg, sizeof(ctrlParms)) == 0)
       {
           if( ctrlParms.string )
           {
               ...
               __copy_to_user(ctrlParms.string, paramValue, 2 - 1);
           }
       ...
       break;
    case BOARD_IOCTL_GET_MANU_MODE:
       if (copy_from_user((void*)&ctrlParms, (void*)arg, sizeof(ctrlParms)) == 0)
       {
           if( ctrlParms.string )
           {
       ...
       break;
    case BOARD_IOCTL_GET_ENABLE_BACKUP_PSI:
       if (copy_from_user((void*)&ctrlParms, (void*)arg, sizeof(ctrlParms)) == 0)
       {
           if( ctrlParms.string )
           {
       ...
       break;
    case BOARD_IOCTL_GET_SYSTEM_LOG_SIZE:
       if (copy_from_user((void*)&ctrlParms, (void*)arg, sizeof(ctrlParms)) == 0)
       {
           if( ctrlParms.string )
           {
       ...
       break;
    case BOARD_IOCTL_GET_AUXILLARY_FILE_SYSTEM_SIZE:
       if (copy_from_user((void*)&ctrlParms, (void*)arg, sizeof(ctrlParms)) == 0)
       {
           if( ctrlParms.string )
           {
       ...
       break;
    case BOARD_IOCTL_GET_MAIN_THREAD_NUMBER:
       if (copy_from_user((void*)&ctrlParms, (void*)arg, sizeof(ctrlParms)) == 0)
       {
           if( ctrlParms.string )
           {
       ...
       break;
    case BOARD_IOCTL_GET_NUMBER_MAC_ADDRESSES:
       if (copy_from_user((void*)&ctrlParms, (void*)arg, sizeof(ctrlParms)) == 0)
       {
           if( ctrlParms.string )
           {
       ...
       break;
    case BOARD_IOCTL_GET_WLANFEATURE:
       if (copy_from_user((void*)&ctrlParms, (void*)arg, sizeof(ctrlParms)) == 0)
       {
           if( ctrlParms.string )
           {
...
#endif
#ifdef AEI_CONSOLE_PASSWD_IN_NVRAM
    case BOARD_IOCTL_GET_CFE_CONSOLE_PASSWORD:
       if (copy_from_user((void*)&ctrlParms, (void*)arg, sizeof(ctrlParms)) == 0)
       {
           if( ctrlParms.string )
           {
               ...
               aei_getNvramValueByParaName(AEI_NVPARAM_CFE_CONSOLE_PASSWORD, paramValue, sizeof(paramValue));
       ...
       break;
#endif
#if defined(AEI_DUAL_IMAGE)
        case BOARD_IOCTL_GET_IMAGE_VERSION :
            if (copy_from_user((void*)&ctrlParms, (void*)arg, sizeof(ctrlParms)) == 0)
            {
                if (ctrlParms.offset == 0) //get image 1
                {                  
                    if (ctrlParms.string)
                    {
                        ...
                        if (NULL != pNvramData)
                        {
                            __copy_to_user(ctrlParms.string, pNvramData->ImageVersion1, NVRAM_IMAGEVERSION_LEN-1);
                            kfree(pNvramData);
                        }
                    }
                    
                }
                else if (ctrlParms.offset == 1) // get image 2
                {
                    if (ctrlParms.string)
                    {
                        NVRAM_DATA *pNvramData = NULL;
                        ...
                        if (NULL != pNvramData)
                        {
                            __copy_to_user(ctrlParms.string, pNvramData->ImageVersion2, NVRAM_IMAGEVERSION_LEN-1);
                            kfree(pNvramData);
                        }
                    }
                }
...
#ifdef CONFIG_ACT_NL_KERNEL
            act_button_notify(NL_BUTTON_ID_WPS, NL_BUTTON_KEYDOWN);
#else
...
#ifdef AEI_WECB
int aei_netlink_broadcast(unsigned int message, unsigned int data);

static int aei_reset_irq = 0;
static unsigned int aei_reset_gpio = 0;

static struct timer_list aei_reset_timer;
static struct timer_list *aei_pTimer = NULL;

#define AEI_RESET_POLL_TIME_MS 100

static Bool aei_reset_button_pressed(unsigned int irq, unsigned int gpio)
{
	unsigned int intSts = 0, extIntr, value = 0;
	int actHigh = 0;
	Bool pressed = 1;
	...
	if( sesBtn_polling == 0 )
	{
...
#endif
#if defined(CONFIG_BCM963138) || defined(CONFIG_BCM963148)
			intSts = kerSysGetGpioValue(MAP_EXT_IRQ_TO_GPIO( irq - INTERRUPT_ID_EXTERNAL_0));
#elif defined(CONFIG_BCM963381)
			intSts = PERF->ExtIrqSts & (1 << (irq - INTERRUPT_ID_EXTERNAL_0 + EI_STATUS_SHFT));
#else
			intSts = PERF->ExtIrqCfg & (1 << (irq - INTERRUPT_ID_EXTERNAL_0 + EI_STATUS_SHFT));
#endif
		...
		extIntr = extIntrInfo[irq - INTERRUPT_ID_EXTERNAL_0];
		actHigh = IsExtIntrTypeActHigh(extIntr);
	...
	else
	{
		...
		if( sesBtn_gpio != BP_NOT_DEFINED )
		{
			actHigh = gpio&BP_ACTIVE_LOW ? 0 : 1;
			value = kerSysGetGpioValue(gpio);
		    if( (value&&actHigh) || (!value&&!actHigh) )
			    pressed = 1;
		}
...
static void aei_restore_to_default_timer_func(unsigned long data)
{

    if(aei_reset_button_pressed(aei_reset_irq, aei_reset_gpio))
    {
        mod_timer(aei_pTimer, jiffies + (HZ / 1000) * AEI_RESET_POLL_TIME_MS);
    }
    else
    {
        aei_netlink_broadcast(AEI_NETLINK_RESET_BUTTON, AEI_BUTTON_RELEASE);
#if defined(CONFIG_BCM963138) || defined(CONFIG_BCM963148)
        BcmHalExternalIrqUnmask(aei_reset_irq);
#else
        BcmHalInterruptEnable(aei_reset_irq);
#endif
    }
}
...
#endif
{
    ...
    if(aei_pTimer == NULL)
    {
        aei_pTimer = &aei_reset_timer;
        init_timer(aei_pTimer);
        aei_pTimer->function = aei_restore_to_default_timer_func;
        aei_pTimer->data = 0;
    }

    aei_pTimer->expires = jiffies + (HZ / 1000) * AEI_RESET_POLL_TIME_MS;
    add_timer(aei_pTimer);

    aei_netlink_broadcast(AEI_NETLINK_RESET_BUTTON, AEI_BUTTON_PRESS);
#else
    char buf[256];
...
#ifdef AEI_WECB
            aei_reset_irq = irq;
            aei_reset_gpio = *(unsigned int *)dev_id;
#else
     	}
#endif
        return IRQ_HANDLED;
...
#if defined(AEI_WECB) && defined(AEI_DUAL_IMAGE)
