...
#ifdef CONFIG_ACT_NL_KERNEL
    {kLedSes, BpGetWirelessSesLedGpio, BpGetWirelessSesLedGpio2},
    {kLedWlan2G_ACT, BpGetWireless2GLedGpioACT, NULL},
    {kLedWlan2G, BpGetWireless2GLedGpio, BpGetWireless2GFailLedGpio},
#else
    {kLedSes, BpGetWirelessSesLedGpio, NULL},
#endif
#ifdef AEI_WECB
    {kLedPower, BpGetBootloaderPowerOnLedGpio, BpGetBootloaderStopLedGpio},
    {kLedWlan5G, NULL, BpGetWireless5GFailLedGpio},
    {kLedPhy1Fail, NULL, BpGetGpioLEDPhy1Fail},
    {kLedPhy2Fail, NULL, BpGetGpioLEDPhy2Fail},
#endif
...
static void setLed (PLED_CTRL pLed, unsigned short led_state, unsigned short led_type)
{
	...
	}else if((led_gpio&BP_GPIO_NUM_MASK)<36){
	}else if((led_gpio&BP_GPIO_NUM_MASK)<52){
		GPIO->GPIOCtrl |= (0x1 << ((led_gpio&BP_GPIO_NUM_MASK)-32)); /*periph control*/
...
static void AEI_ledToggle_by_gpio(short led_gpio)
{
#if defined(CONFIG_BCM96328) || defined(CONFIG_BCM96362) || defined(CONFIG_BCM963268) || defined(CONFIG_BCM96318)
    LED->ledMode ^= (LED_MODE_MASK << GPIO_NUM_TO_LED_MODE_SHIFT(led_gpio));
...
// toggle the LED
static void AEI_ledToggle(PLED_CTRL pLed)
{
   ...
   red_led_gpio = pLed->ledRedGpio ;
    ...
    if (-1 != green_led_gpio)
    {
        AEI_ledToggle_by_gpio(green_led_gpio);
    }

    if (-1 != red_led_gpio)
    {
        AEI_ledToggle_by_gpio(red_led_gpio);
    }
}
...
#ifdef AEI_WECB
        case kLedStateAllBlink:               /* 250ms on, 250ms off */
            if (pCurLed->blinkCountDown-- == 0)
            {
                pCurLed->blinkCountDown = kSlowBlinkCount;
                AEI_ledToggle(pCurLed);
            }
            gTimerOnRequests++;
            spin_unlock_irqrestore(&brcm_ledlock, flags);
            break;
...
#ifdef CONFIG_ACT_NL_KERNEL
static void set_led_gpio(unsigned int led_id, unsigned int on)
{
	static unsigned int act_is_active = 0;

	switch(led_id){
		case NL_LED_ID_WPS_GREEN:
			setLed(&gLedCtrl[kLedSes], on?kLedOn:kLedOff, kLedGreen);
			break;
		case NL_LED_ID_WPS_RED:
			setLed(&gLedCtrl[kLedSes], on?kLedOn:kLedOff, kLedRed);
			break;
		case NL_LED_ID_2G:
			if(0==act_is_active){
				kerSysLedCtrl(kLedWlan2G_ACT, kLedStateOn);
				act_is_active = 1;
				mdelay(1);
			}
			kerSysLedCtrl(kLedWlan2G, on?kLedStateOn:kLedStateOff);
			break;
		case NL_LED_ID_5G:
//			kerSysLedCtrl(kLedWlan5G, on?kLedStateOn:kLedStateOff);
			break;
		default:
			break;
	}
...
led_op_cb g_led_op_cb = {
	.set_led_gpio = set_led_gpio,
};
