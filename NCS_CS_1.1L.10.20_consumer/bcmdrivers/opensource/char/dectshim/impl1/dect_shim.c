...
/* 63148 MEMC can access greater tham 256MB of memory 
 * Need to restrict range of DMA memory for DECT DMA  
 * to within the first 256MB, because DECT DMA can only
 * access first 256MB of memory. NOTE: This needs
 * ZONE_DMA support to be compiled in the kernel
 */
#define DECT_DMA_VALID_DDR_ADDR_BITS          28
#else
/* 63138 MEMC can only access upto 256MB of memory
 * No need to restrict the range of DMA memory for
 * DECT DMA, since it can never exceed 256MB */
#define DECT_DMA_VALID_DDR_ADDR_BITS          32  
#endif /* CONFIG_BCM963148 */
