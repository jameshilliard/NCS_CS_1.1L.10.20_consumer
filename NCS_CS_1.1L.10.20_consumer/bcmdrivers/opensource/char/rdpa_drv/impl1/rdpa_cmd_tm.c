            ...
            uint32_t weight)
{
	...
	if (pq->alloc)  {
		CMD_TM_LOG_ERROR("FAIL: port_id=%d : q_id(%u) is already  ALLOCATED", port_id, q_id);
		return RDPA_DRV_ERROR;
	}
...
int rdpa_cmd_tm_ioctl(unsigned long arg)
{
    ...
    switch(tm.cmd)
    {
        ...
        case RDPA_IOCTL_TM_CMD_GET_QUEUE_CONFIG: {
            CMD_TM_LOG_INFO("RDPA_IOCTL_TM_CMD_GET_QUEUE_CONFIG: ROOT_tm(%u) dev_type(%d) dev_id(%d) q_id(%u)", 
                tm.root_tm_id, tm.dev_type, tm.dev_id, tm.q_id);
