...
#if defined(CONFIG_BCM963138) || defined(CONFIG_BCM963148)
    /* XXX: FIXME */
    if (is_ext_sw && dev->base_addr < MAX_NUM_OF_VPORTS)
        goto Exit;
#endif

    *rdpa_port = rdpa_if_lan0 + (dev->base_addr > (MAX_NUM_OF_VPORTS - 1) ?
        dev->base_addr - MAX_NUM_OF_VPORTS : dev->base_addr);
#if defined(CONFIG_BCM963138) || defined(CONFIG_BCM963148)
    ...
    if (LOGICAL_PORT_TO_PHYSICAL_PORT(dev->base_addr) == g9991_bp_debug_port)
    {
    ...
    if (!is_ext_sw)
    {
        ...
        port_cfg.emac = (rdpa_emac)dev->base_addr;
        rdpa_port_cfg_set(rdpa_port_attrs, &port_cfg);
        port_emac_cfg.enable = 1;
        rdpa_port_emac_cfg_set(rdpa_port_attrs, &port_emac_cfg);
    }
...
/* 63138 has 2x4 mux */
#define SF2_NUM_INT_PORT_ON_MUX     (2) 
#define SF2_NUM_EXT_PORT_ON_MUX     (4) 
#define SF2_EXT_PORT_NUM_BASE       (9) /* Base port number for external ports */
#define SF2_EXT_PORT_NUM_MAX        (SF2_EXT_PORT_NUM_BASE+SF2_NUM_EXT_PORT_ON_MUX-1) /* Max port number for external ports */
#define SF2_RNR_WAN_INT_PORT_ON_MUX (1) /* Mux internal port that connects to Runner */

#define SF2_VALID_CB_INT_PORT(x) ( ((x) >= 0) && ((x) < SF2_NUM_INT_PORT_ON_MUX) )
#define SF2_VALID_CB_EXT_PORT(x) ( ((x) >= SF2_EXT_PORT_NUM_BASE) && ((x) <= SF2_EXT_PORT_NUM_MAX) )
#define SF2_CB_EXT_PORT_TO_IDX(x) ( (x) - SF2_EXT_PORT_NUM_BASE ) /* Array index for a given cross bar external port */
#define SF2_CB_EXT_PORT_FROM_IDX(x) ( (x) + SF2_EXT_PORT_NUM_BASE ) /* Cross bar external port for a given array index */

/*
 * ext_port is one of 4 phys/ports available on Crossbar
 * int_port is one of the 2 ports available on Crossbas (1 port for Runner P_WAN and others for switch) 
 */
static int crossbar_select(int ext_port, int int_port)
{
    u32 val32 = 0;
    volatile u32 *cb_mux_reg = (void *)(SWITCH_BASE + CROSSBAR_SWITCH_REG);
    if (!SF2_VALID_CB_INT_PORT(int_port)) {
        return -BCM_E_PARAM;
    }
    val32 =  *cb_mux_reg;
    val32 &= ~(CB_PHY_PORT_MASK << (int_port * CB_PHY_PORT_SHIFT));
...
static enetSwPort_t muxInt2SwPort[SF2_NUM_INT_PORT_ON_MUX] = {{1,4}, {0, 0}};
static int swPort2MuxInt[BP_MAX_ENET_MACS][BP_MAX_SWITCH_PORTS] = 
    {[0 ... (BP_MAX_ENET_MACS-1)][0 ... (BP_MAX_SWITCH_PORTS-1)] = -1};

/* Crossbar Internal <-> External Port Configuration Array */
static int muxInt2Ext[SF2_NUM_INT_PORT_ON_MUX] = {[0 ... (SF2_NUM_INT_PORT_ON_MUX-1)] = BP_CROSSBAR_NOT_DEFINED};
static int muxExt2Int[SF2_NUM_EXT_PORT_ON_MUX] = {[0 ... (SF2_NUM_EXT_PORT_ON_MUX-1)] = BP_CROSSBAR_NOT_DEFINED};

static void config_crossbar(void)
{
    ETHERNET_MAC_INFO *info = EnetGetEthernetMacInfo();
    int sw_port, sw_unit, muxExtPort, muxIntPort;

    for (muxIntPort = 0; muxIntPort < SF2_NUM_INT_PORT_ON_MUX; muxIntPort++)
    {
    ...
    {
        for (muxExtPort = 0; muxExtPort < BP_MAX_CROSSBAR_PORTS; muxExtPort++) {
            ...
            if (muxExt2Int[muxExtPort] != -1)
            {
                printk("\nERROR : Duplicate MUX mapping <%d:%d> and <%d:%d>\n\n",
                    muxExt2Int[muxExtPort], muxExtPort, muxIntPort, muxExtPort);
                continue;
            }
    ...
    for (muxIntPort = 0; muxIntPort < SF2_NUM_INT_PORT_ON_MUX; muxIntPort++) {
        if (muxInt2Ext[muxIntPort] == -1) { /* Not configured through Board params */
            /* Find unused port */
            for (muxExtPort = 0; muxExtPort < SF2_NUM_EXT_PORT_ON_MUX; muxExtPort++) {
    ...
    *new_cb_port = BP_CROSSBAR_NOT_DEFINED; /* Initialize */
    ...
    for(muxExtPort = 0; muxExtPort < BP_MAX_CROSSBAR_PORTS; muxExtPort++)
    {
        ...
        if(phys.lnk)
        {
            ...
            muxInt2Ext[muxIntPort] = muxExtPort;
            info->sw.phy_id[phsclPort] = phyId;
...
void bcmeapi_update_link_status(void)
{
    ...
    if (port_map) 
    {
        ...
        for (vport = vport_cnt; port_map && vport >= 1; vport--) /* Starting backward - internal switch port are towards end */
        {
            ...
            if (LOGICAL_PORT_TO_UNIT_NUMBER(logPort) == 0) /* Internal switch i.e. Runner port */
            {
