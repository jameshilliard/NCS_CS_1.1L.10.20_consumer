...
int cbport_to_vportIdx[BP_MAX_CROSSBAR_PORTS] = {[0 ... (BP_MAX_CROSSBAR_PORTS-1)] = -1};
#endif
...
static int create_vport(void)
{
...
#endif /* */
#if defined(CONFIG_AEI_QUANTENNA_RGMII)
    ...
    for ( vport_id = 1, logical_port = 0 ; vport_id < vport_cnt + 1; vport_id++, logical_port++, (*map_p) /= 2)
    {
        ...
        do
        {
            if (cb_port >= 0 && cb_port < BP_MAX_CROSSBAR_PORTS) /* Range check */
            {
...
#if defined(CONFIG_AEI_QUANTENNA_RGMII)
	    else if(phy_id == LAN_PORT_ID_2)
        {
	        //eth2 is dedicated to RGMII port2 interface
	        sprintf(ifname, "eth%d",phy_id);
...
#if defined(CONFIG_AEI_QUANTENNA_RGMII)
	/* Link up RGMII port */
	v8 = REG_PORT_STATE_LNK | REG_PORT_STATE_FDX | REG_PORT_STATE_1000 | REG_PORT_STATE_FLOWCTL | REG_PORT_STATE_OVERRIDE;
	extsw_wreg(PAGE_CONTROL, REG_PORT2_STATE, &v8, 1);
#endif
...
void link_change_handler(int port, int cb_port, int linkstatus, int speed, int duplex)
{
    ...
    if (pNetDev) 
    {
        ...
        if (cb_port != BP_CROSSBAR_NOT_DEFINED && cb_port >= 0 && cb_port < BP_MAX_CROSSBAR_PORTS)
        {
...
#if defined(CONFIG_AEI_QUANTENNA_RGMII)
		/* Disable the FAP TM on eth2*/
               if(strcmp(pNetDev->name,"eth2")== 0)
               {
...
#if defined (CONFIG_BCM_JUMBO_FRAME)
    {
        int we_locked_rtnl = FALSE;
            ...
            printk((KERN_CRIT "leo dbg:%s (%s switch port: %d) (Logical Port: %d) Link DOWN.\n"),
                    pNetDev->name, (LOGICAL_PORT_TO_UNIT_NUMBER(port)?"Ext":"Int"),LOGICAL_PORT_TO_PHYSICAL_PORT(port), port);
#if defined(AEI_WECB) && defined(AEI_ETH_PORT_UNPLUG_NOTIFY)
            aei_netlink_broadcast(AEI_NET_PORT_LINK_DOWN, port);
#endif
            /* notify linux after we have finished setting our internal state */
            if (netif_carrier_ok(pNetDev) != 0)
    ...
    while (atomic_read(&poll_lock) > 0)
    {
        ...
        if((linkChanged = bcmenet_link_might_changed()))
        {
            for (i = 1; i <= vport_cnt; i++)
            {
                ...
                phys = ethsw_phy_stat(LOGICAL_PORT_TO_PHYSICAL_PORT(log_port));
...
int enet_sw_port_to_phyid(int unit, int phy_port)
{
    ...
    return pDevCtrl->EnetInfo[unit].sw.phy_id[phy_port];
}
...
int enet_get_first_crossbar_port(int logPort)
{
    ...
    for(/* muxExtPort init above*/; muxExtPort < BP_MAX_CROSSBAR_PORTS; muxExtPort++)
    {
