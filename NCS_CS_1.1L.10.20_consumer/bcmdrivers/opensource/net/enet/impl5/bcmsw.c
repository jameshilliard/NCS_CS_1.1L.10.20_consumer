...
void extsw_port_trunk_init(void)
{
    ...
    for (port = 0; port_map && (port < BP_MAX_PHY_PORTS); port++) /* go through all the ports including crossbar */
    {
        if (BP_IS_CROSSBAR_PORT(port)) /* Crossbar port ? Find the port_flags and actual switch port */
        {
            ...
            if (info->sw.crossbar[crossbar_port].switch_port == BP_CROSSBAR_NOT_DEFINED) continue;
...
inline static void swab64_mem(char *ptr)
{
    /* Assume ptr starts in 2byte alightment */
    *((uint16 *)(ptr + 0)) = __cpu_to_le16(*((uint16 *)(ptr + 0)));
    *((uint16 *)(ptr + 2)) = __cpu_to_le16(*((uint16 *)(ptr + 2)));
    *((uint16 *)(ptr + 4)) = __cpu_to_le16(*((uint16 *)(ptr + 4)));
#if defined(CONFIG_CPU_BIG_ENDIAN)
    *((uint16 *)(ptr + 0)) ^= *((uint16 *)(ptr + 4));
    *((uint16 *)(ptr + 4)) ^= *((uint16 *)(ptr + 0));
    *((uint16 *)(ptr + 0)) ^= *((uint16 *)(ptr + 4));
#endif
      ...
      *((uint32 *)data) = __le32_to_cpu(*((uint32 *)val));
      break;
        case 6:
            ...
            swab64_mem(val);
            if (type) {
                /* a 64 bit quantity to return, of which least 48 bits significant */
                *(uint64 *)data = 0;
                memcpy(data, val, 6);
    #if defined(CONFIG_CPU_BIG_ENDIAN)
                (*(uint64 *)data) >>= 16;               
   ...
   case 4:
      *((uint32 *)val) = __cpu_to_le32(*((uint32 *)data));
      break;
        case 6:
            if (type) {
/* a 64 bit quantity handed to you from which to pick least 48 bits */
                memcpy(val, data, 8);
#if defined(CONFIG_CPU_BIG_ENDIAN)
                (*(uint64 *)val) <<= 16;
#endif
        ...
        case 8:
            switch (type)
            {
                case 0:
                    ...
                    *((uint32 *)&val[0]) = swab32(*((uint32 *)&data[4]));
                    *((uint32 *)&val[4]) = swab32(*((uint32 *)&data[0]));
...
int enet_arl_read_ext( uint8_t *mac, uint32_t *vid, uint32_t *val )
{
            ...
            extsw_rreg_wrap(PAGE_AVTBL_ACCESS, REG_ARL_SRCH_CTRL_531xx, (uint8_t *)&v8, 1))
    {
        ...
        /* Found a valid entry */
        extsw_rreg_wrap(PAGE_AVTBL_ACCESS, REG_ARL_SRCH_MAC_LO_ENTRY_531xx,&mac_vid_v64[0], 8);
...
static void enet_arl_access_prepare_ext(uint8_t *mac, uint16_t vid)
{
    ...
    extsw_wreg_wrap(PAGE_AVTBL_ACCESS, REG_ARL_MAC_INDX_LO, (uint8_t *)&mac[0], 6);
...
void enet_arl_write_ext( uint8_t *mac, uint16_t vid, uint32_t val )
{
    ...
    *(uint16 *)(&mac_vid_v64[0]) = (vid & 0xFFF);
    ...
    /* write MAC and VID */
    extsw_wreg_wrap(PAGE_AVTBL_ACCESS, REG_ARL_MAC_LO_ENTRY, (uint8_t *)&mac_vid_v64, 8);
...
int enet_arl_access_dump_ext(void)
{
            ...
            ++count, extsw_rreg_wrap(PAGE_AVTBL_ACCESS, REG_ARL_SRCH_CTRL_531xx, (uint8_t *)&v8, 1))
    {
        ...
        /* Found a valid entry */
        extsw_rreg_wrap(PAGE_AVTBL_ACCESS, REG_ARL_SRCH_MAC_LO_ENTRY_531xx,&mac_vid[0], 8);
        BCM_ENET_DEBUG("ARL_SRCH_result (%02x%02x%02x%02x%02x%02x%02x%02x) \n",
...
void enet_arl_dump_ext_multiport_arl(void)
{
    ...
    if (!enabled)
    {
        ...
        for (i=0; i<6; i++)
        {
        extsw_rreg_wrap(PAGE_ARLCTRL, REG_MULTIPORT_ADDR1_LO + i*16, (uint8 *)&addr, sizeof(addr));
...
int remove_arl_entry_ext(uint8_t *mac)
{
    ...
    while (v8 & ARL_SRCH_CTRL_START_DONE) {
        ...
        { /* found the matching entry ; invalidate it */
            ...
            /* There is still bug on MDIO 8 byte register access driver, so we
                have to write 2 bytes after 8 bytes to work around before the bug is fixed. */
            extsw_wreg_wrap(PAGE_AVTBL_ACCESS, REG_ARL_MAC_INDX_LO, (uint8_t *)&mac_vid_v64[0], 8);
            extsw_wreg_wrap(PAGE_AVTBL_ACCESS, REG_ARL_VLAN_INDX, (uint8_t *)&mac_vid_v64[6], 2);
            ...
            extsw_rreg_wrap(PAGE_AVTBL_ACCESS, REG_ARL_MAC_LO_ENTRY,&mac_vid_v64[0], 8);
            BCM_ENET_DEBUG("ARL_SRCH_result (%02x%02x%02x%02x%02x%02x%02x%02x) \n",
                           mac_vid_v64[0],mac_vid_v64[1],mac_vid_v64[2],mac_vid_v64[3],mac_vid_v64[4],mac_vid_v64[5],mac_vid_v64[6],mac_vid_v64[7]);
            /* Compare the MAC */
...
int bcmsw_dump_mib_ext(int port, int type)
{
    ...
    if (type) 
    {
        extsw_rreg_wrap(PAGE_MIB_P0 + (port), REG_MIB_P0_TXOCTETS, data, SWAP_TYPE1|8);
        v32 = (((uint32*)data)[0]);                                
...
int bcmsw_set_multiport_address_ext(uint8_t* addr)
{
    if (bcm63xx_enet_isExtSwPresent())
    {
        ...
        for ( i = 0; i < MULTIPORT_CTRL_COUNT; i++ )
        {
           *(uint16*)(&v64[0]) = 0;
           memcpy(&v64[2], addr, 6);
           extsw_rreg_wrap(PAGE_ARLCTRL, (REG_MULTIPORT_ADDR1_LO + (i * 0x10)), (uint8 *)&cur64, sizeof(cur64));
        ...
        for ( i = 0; i < MULTIPORT_CTRL_COUNT; i++ )
        {
            ...
            if ( 0 == (v16 & (MULTIPORT_CTRL_EN_M << (i << 1))))
            {
                ...
                *(uint16*)(&v64[0]) = 0;
                memcpy(&v64[2], addr, 6);
                extsw_wreg_wrap(PAGE_ARLCTRL, (REG_MULTIPORT_ADDR1_LO + (i * 0x10)), (uint8 *)&v64, sizeof(v64));
                v32 = PBMAP_MIPS;
...
int bcmsw_get_hw_stats(int port, int extswitch, struct net_device_stats *stats)
{
    ...
    if (extswitch) {
        ...
        // Track RX byte count
        extsw_rreg_wrap(PAGE_MIB_P0 + (port), REG_MIB_P0_RXOCTETS + REG_MIB_P0_EXTSWITCH_OFFSET, data, SWAP_TYPE1|8);
...
int bcmeapi_ioctl_extsw_dscp_to_priority_mapping(struct ethswctl_data *e)
{
    ...
    if (e->type == TYPE_GET) {
        extsw_rreg_wrap(PAGE_QOS, REG_QOS_DSCP_PRIO_MAP0LO + mapnum * QOS_DSCP_MAP_REG_SZ,
                                 (void *)&val64, QOS_DSCP_MAP_REG_SZ | SWAP_TYPE1);
...
static int sf2_bcmsw_dump_mib_ext(int port, int type)
{
    ...
    if (type) 
    {
        extsw_rreg_wrap(PAGE_MIB_P0 + (port), SF2_REG_MIB_P0_TXOCTETS, data, SWAP_TYPE1|8);
        v32 = (((uint32*)data)[0]);                                
        printk("TxOctetsLo:             %10u \n", v32);
        v32 = (((uint32*)data)[1]);                                
        printk("TxOctetsHi:             %10u \n", v32);
...
void sf2_rgmii_config(void)
{
    ...
    for (unit=0; unit < BP_MAX_ENET_MACS; unit++) 
    {
        ...
        for(phy_port = 0; phy_port < BP_MAX_PHY_PORTS; phy_port++)
        {
...
int sf2_cosq_sched(struct ethswctl_data *e)
{
    ...
    if (e->type == TYPE_GET) {
        ...
        extsw_rreg_wrap(PAGE_QOS_SCHEDULER, REG_PN_QOS_WEIGHT_PORT_0 +
                       e->port * REG_PN_QOS_WEIGHTS, (uint8_t *)data, SWAP_TYPE1|REG_PN_QOS_WEIGHTS);
 ...
 // programming queue weights.
        if (e->port_qos_sched.num_spq || e->port_qos_sched.sched_mode == BCM_COSQ_WRR) {
            ...
            extsw_wreg_wrap(PAGE_QOS_SCHEDULER, REG_PN_QOS_WEIGHT_PORT_0 + e->port * REG_PN_QOS_WEIGHTS,
                            (uint8_t *)data, SWAP_TYPE1 | REG_PN_QOS_WEIGHTS);
