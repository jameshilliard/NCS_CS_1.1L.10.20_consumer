...
void robosw_configure_ports()
{
    ...
    for (i = 0; i < EPHY_PORTS; i++) {
        ...
        if (i == AEI_QTN_RGMII_PORT_IDX) {
            printk("RoboSW: set Quantenna RGMII port %d to VLAN with CPU port only\n",
                   i);
...
#define LNK_LED_DIS  4 // Bit4
    ...
    for (i=0; i<4; i++) {
        ethsw_phy_rreg(ethsw_port_to_phyid(i), AUX_MODE_REG, &v16);
        ...
        ethsw_phy_wreg(ethsw_port_to_phyid(i), AUX_MODE_REG, &v16);
    }
