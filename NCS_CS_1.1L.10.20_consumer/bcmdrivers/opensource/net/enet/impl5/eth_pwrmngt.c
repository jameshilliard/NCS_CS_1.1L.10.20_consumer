...
int ethsw_reset_phys(void)
{
    ...
    for (i = 0; i < MAX_TOTAL_SWITCH_PORTS; i++) {
        if ((portmap & (1U<<i)) != 0) {
            if (BpGetPhyResetGpio(i, &pin) == BP_SUCCESS && (pin & BP_GPIO_NUM_MASK) < GPIO_NUM_MAX)
            {
		printk("************************* resest pin 0x%x\n", pin);
                kerSysSetGpioState(pin,  kGpioInactive);
                mdelay(1);
                kerSysSetGpioState(pin,  kGpioActive);
                mdelay(350);
            }
        }
    }
