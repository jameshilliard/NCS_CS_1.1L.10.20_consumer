...
static int wfd_tasklet_handler(void *context)
{
    ...
    while (1)
    {
        wait_event_interruptible(wfd_rx_thread_wqh, wfd_rx_work_avail);
...
static int wfd_dev_init(void)
{
    ...
    printk ("\033[1m\033[34m %s is initialized !\n\r\033[0m", version);
        
    return 0;

proc_release:
    remove_proc_entry("wfd", proc_directory);
             ...
             HOOKV wfd_completeHook)
{
    ...
    if (qidx < WFD_NUM_QUEUE_SUPPORTED)
    {
        ...
        printk("\033[1m\033[34m %s: Dev %s wfd_idx %d Type %s configured WFD CPU "
               "RxQId (%d), status (%d) number_of_queues %d qmask 0x%x\n\033[0m", __FUNCTION__, 
               wl_dev_p->name, (wfd_idx-1), 
